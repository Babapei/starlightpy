...
不是论文。是一门（或一个课题）的**软件作业**：STARLIGHT 那个 Fortran 程序难用，做成 Python 库，天文学家可以 `import` 之后在代码里拟合，不用再啃 grid 文件。用户是天文学家，交付物是库，不是文章。

今天再看：同一句「方便」已经被 pPXF 做掉了。作业原意仍然成立，但不必再自研一套求解器；要做方便，是把已有拟合器接到数据上。本仓库现有代码适合当合成谱 / 教学模型，不是对外产品。

## 建议你怎么选

1. **对上当年作业**：Python 里三行调用、读谱、出成分和消光。不必对齐 Fortran，更不必发文。
2. **现在还想做「方便」**：薄封装已有拟合器（pPXF）+ 巡天读入，而不是继续自研 NNLS。
3. **只是翻旧仓库看一眼**：读完这份说明即可，不必维护。
...
所以本仓库不追求「成为拟合标准」。物理模型可以留着造合成谱；若还要做「方便天文学家」，接已有拟合器，而不是再写求解器或整合各家码。
