"""Readers for STARLIGHT-style ASCII spectrum, mask and base files."""
...
def load_spectrum(
    ...
    """Load λ, flux, error, flag from a 2–4 column ASCII spectrum."""
...
def load_base_master(master_path: PathLike) -> List[dict]:
    """Parse a STARLIGHT base master file (e.g. Base.BC03.N)."""
...
def load_base_spectra(
    base_dir: PathLike,
    base_list: Sequence[dict],
) -> Tuple[NDArray[np.float64], NDArray[np.float64]]:
    """Load each base spectrum and stack them as (n_wave, n_components)."""
...
def load_mask(filename: PathLike) -> List[Tuple[float, float, float]]:
    """Read a STARLIGHT mask file: Nmasks, then λ_ini λ_fin weight rows."""
    with open(filename, "r", encoding="utf-8") as handle:
        lines = [line for line in handle.readlines() if line.strip() and not line.startswith("#")]
    if not lines:
...
def apply_mask(
    ...
    """True where the pixel is usable (not covered by a mask interval)."""
