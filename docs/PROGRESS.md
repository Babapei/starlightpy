...
| --- | --- |
| 阶段 | G（可认真调用的 1.x） |
| 小段 | **下一刀 G1**（FitResult + 防错）；v0.1 = A–E 已完成 |
...
未开始 G1 代码。实现后走 `fit_spectrum`，断言放在唯一文件名的测试里（例如 `tests/starlightpy/test_starlight_g1.py`）。
...
| 2026-09-02 | 合同 | PLAN 改为全面好的 1.x；断点 G1；未开始 G1 代码 |
