...
最后更新：2026-09-06（1.0.0 已上 PyPI；README 主安装句为 pip install）
...
| 编号 | 项 | 怎么做才算还是这个库 | 不要做成 |
| --- | --- | --- | --- |
| O4 | PyPI / DOI / 版本节奏 | 包名 `starlightpy`；安装说明与仓库一致。不改拟合物理。**1.0.0 已上 PyPI**（https://pypi.org/project/starlightpy/）。DOI / Zenodo 仍可选，未做。 | 借发行改模型或捆绑 SSP |
...
- [x] **O4-PyPI** `starlightpy` 1.0.0 已上 PyPI；README 主安装句为 `pip install starlightpy`。GitHub tag 安装留作备选。不做 Zenodo。不改拟合。
...
见 [docs/PROGRESS.md](PROGRESS.md)。`starlightpy` 1.0.0 已上 PyPI。§2 以后可做其余项**不要自动开工**。下一步若做代码，从 PROGRESS 点名的一项开始（建议先 O1 或 O2），先写验收再编码。
