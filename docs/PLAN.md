...
- **不会把 pPXF 封装放进同一个库。** 那是另一个产品。现在的 `easyppxf` 可以留着不动，但不要为它对齐 API、不要做成 `backend=`。新功能不要往那边加。
...
现在不必清空重来：阶段 A 的前向模型和 NNLS 与从头设计一致。开工后按「从头会做的那几条」写阶段 B，把「从头不会做的」只当遗留、不扩张。
