...
G（1.x）已完成。阶段 H 进行中：一次一个小段，默认关，先写验收再编码。
...
| `io.py` | `.cxt` / mask / base / gzip / FITS / `iter_ascii_spectra` / `resample_to` | 有 | H6：npz/json 存盘（不是 Fortran `.out`） |
...
- [ ] **H6** 自己的 npz/json 存盘（不是 Fortran `.out`）
...
**阶段 H 预告（实现对应小段时才写进代码）：**

- `save_fit_result` / `load_fit_result`（H6；npz 或 json。不是 Fortran `.out`。默认不自动写盘）
...
1. **一次只做一件。** 当前只做 PROGRESS 里的 H 小段。不要同时改 `easyppxf`、不要为「更像 Fortran」加开关。
...
见 [docs/PROGRESS.md](PROGRESS.md)。当前小段是 **H6**（npz/json 存盘）。未完成 H6 不要开 H 以外的功能。
